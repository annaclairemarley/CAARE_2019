...
chuska_area_mm = 4581
bm_area_mm = 1413 
defiance_area_mm = 1106
mt_powell_area_mm = 77
nm_area_mm = 96
nn_area_mm = 67636
```
...
# first need to change mm to km, then multiply by their area in km2
car_an_total_swe <- car_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*carrizo_area_km)

ch_an_total_swe <-ch_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*chuska_area_km)

bm_an_total_swe <-bm_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*bm_area_km)

mp_an_total_swe <-mp_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*mt_powell_area_km)

dp_an_total_swe <-dp_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*defiance_area_km)

nm_an_total_swe <-nm_an_swe %>% 
  mutate(swe_km = swe_mm/1000000) %>% 
  mutate(swe_total_km3 = swe_km*nm_area_km)

# then join the dataframes together
# graph annual average stacked on top of each other
# join together
an_av_total <- join_months_km(carrizo = car_an_total_swe, chuska = ch_an_total_swe, 
...
an_av_total_swe_graph <- an_av_total %>% 
  ggplot(aes(x = waterYear, y = swe_total_km3)) +
  geom_col(aes(fill = region)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(breaks = seq(2004, 2019, by = 2)) +
  scale_fill_discrete(name = "Region", labels = c("Carrizo", "Chuska", "Black Mesa", "Mt. Powell", "Defiance Plateau", "Navajo Mt.")) +
  labs(
    x = "Water Year",
    y = "SWE (km^3)",
    title = "Total SWE in each Region"
  ) +
  coord_flip()+
  theme_classic()
...
contributions <- data.frame(car_an_total_swe$waterYear, 
                       car_an_total_swe$swe_total_km3, 
                       ch_an_total_swe$swe_total_km3,
                       bm_an_total_swe$swe_total_km3,
                       dp_an_total_swe$swe_total_km3,
                       nm_an_total_swe$swe_total_km3,
                       mp_an_total_swe$swe_total_km3) %>% 
                rename(waterYear = car_an_total_swe.waterYear,
                       car_swe_km3 = car_an_total_swe.swe_total_km3,
                       ch_swe_km3 = ch_an_total_swe.swe_total_km3,
                       bm_swe_km3 = bm_an_total_swe.swe_total_km3,
                       nm_swe_km3 = nm_an_total_swe.swe_total_km3,
                       mp_swe_km3 = mp_an_total_swe.swe_total_km3,
                       dp_swe_km3 = dp_an_total_swe.swe_total_km3) %>% 
              mutate(mt_range_sum_total = rowSums(.[, names(.) != "waterYear"])) %>% 
              mutate(car_cont = car_swe_km3/mt_range_sum_total,
                     ch_cont = ch_swe_km3/mt_range_sum_total,
                     bm_cont = bm_swe_km3/mt_range_sum_total,
                     mp_cont = mp_swe_km3/mt_range_sum_total,
                     nm_cont = nm_swe_km3/mt_range_sum_total,
                     dp_cont = dp_swe_km3/mt_range_sum_total)
